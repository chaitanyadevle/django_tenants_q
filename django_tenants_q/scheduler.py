                ...
                with db.transaction.atomic(using=db.router.db_for_write(Schedule)):
                    for s in (
                        ...
                        if s.schedule_type != s.Once:
                            ...
                            s.next_run = next_run
                            s.repeats += 1
                        # send it to the custer; any cluster name is allowed in multi-queue scenarios
                        ...
                        if not s.task:
                            logger.error(
                                _(
                                    "%(process_name)s failed to create a task from schedule [%(scehdule)s] under tenant %(schema_name)s"
                                )
                                % {
                                    "process_name": current_process().name,
                                    "scehdule": s.name or s.id,
                                    "schema_name" :kwargs.get('schema_name', 'UNSPECIFIED'),
                                }
                            )
                        else:
                            logger.info(
                                _(
                                    "%(process_name)s created a task from schedule [%(scehdule)s] under tenant %(schema_name)s"
                                )
                                % {
                                    "process_name": current_process().name,
                                    "scehdule": s.name or s.id,
